...
import { ArrowRight, BookOpenCheck, ClipboardList, ClipboardPlus, Sparkles } from "lucide-react";
...
export default function Home() {
  return (
    <div className="theme-page min-h-screen">
      <PlatformHeader />
      <main>
        <section className="container py-3 sm:py-4"><TutorTelegramBanner /></section>
        <section className="obsidian-grid relative overflow-hidden border-b border-white/10">
          ...
          <div className="container relative grid items-start gap-10 py-10 sm:py-12 lg:grid-cols-[1.15fr_.85fr] lg:py-16">
            <div>
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                <Link href="/tutor"><Button size="lg" className="rounded-xl bg-[#ff5b14] px-6 font-extrabold text-[#101014] hover:bg-[#ff7a35]"><ClipboardPlus className="mr-2 h-4 w-4" />Добавить задание</Button></Link>
                <Link href="/bank"><Button size="lg" variant="outline" className="rounded-xl border-white/18 bg-white/5 px-6 font-extrabold text-[#f5f0e9] hover:bg-white/10">Найти задание<ArrowRight className="ml-2 h-4 w-4" /></Button></Link>
              </div>
              <p className="mt-7 text-sm font-medium text-[#8e8b91]">Без регистрации · без навязчивой структуры · с разбором по запросу</p>
            </div>
            <div className="grid gap-3">
              <Link href="/tutor"><article className="surface-card group rounded-2xl p-6 transition hover:border-[#ff5b14]/55"><ClipboardPlus className="h-7 w-7 text-[#ff5b14]" /><h2 className="mt-5 text-2xl font-bold">Добавить задание</h2><p className="theme-muted mt-2 text-sm leading-6">Выберите ученика и нужные задачи из банка — без поиска по разделам.</p><span className="mt-5 inline-flex items-center text-sm font-bold text-[#ff8b4b]">Создать задание <ArrowRight className="ml-2 h-4 w-4" /></span></article></Link>
