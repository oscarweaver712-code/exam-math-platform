...
export default function TutorWorkspace() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0b0d] text-[#f5f0e9]">
      <PlatformHeader />
      <main className="container py-8 sm:py-10 lg:pl-64">
        <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-[#151518] p-6 sm:p-8">
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-[#ff7a35]">Быстрое действие / математика ОГЭ</p><h1 className="mt-4 text-4xl font-bold tracking-[-.055em] sm:text-5xl">Добавить задание</h1><p className="mt-4 max-w-xl text-sm leading-6 text-[#aaa7ae]">Сначала выберите ученика и задачи. Название и срок уже можно оставить по умолчанию.</p></div>
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[1.18fr_.82fr]">
          <section className={panel}>
            <div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#ff5b14] text-[#101014]"><MailPlus className="h-4 w-4" /></span><h2 className="text-lg font-bold">Пригласить ученика</h2></div><p className="mt-3 text-sm leading-6 text-[#aaa7ae]">Подключите ученика, если его ещё нет в списке.</p><form className="mt-5 space-y-3" onSubmit={event => { event.preventDefault(); createInvite.mutate({ studentEmail: email }); }}><Label htmlFor="student-email" className="text-[#c7c3ca]">Почта ученика</Label><Input id="student-email" type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="student@example.com" required className="h-11" /><Button type="submit" disabled={createInvite.isPending} className={`w-full ${orangeButton}`}>Создать код приглашения</Button></form>
