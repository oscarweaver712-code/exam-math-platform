...
export function CabinetSidebar({ learningRole }: { learningRole?: "student" | "tutor" }) {
  ...
  return <aside className="theme-surface hidden self-start rounded-2xl border p-3 lg:sticky lg:top-24 lg:block"><div className="flex items-center gap-2 px-3 pb-3 pt-1"><UserRound className="h-4 w-4 text-[#ff7a35]" /><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#77747b]">Личный кабинет</p></div><nav className="space-y-2" aria-label="Разделы личного кабинета"><NavigationGroup title="Учёба" items={learning} location={location} />{roleItems.length ? <NavigationGroup title="Роль" items={roleItems} location={location} /> : null}<NavigationGroup title="Аккаунт" items={account} location={location} defaultOpen={location.startsWith("/settings")} /></nav></aside>;
}
