...
import { BookOpenCheck, ClipboardList, Compass, FileCheck2, GraduationCap, Home, Image, Inbox, LayoutDashboard, Library, Menu, Moon, Settings, ShieldCheck, SlidersHorizontal, Sun, UserRound } from "lucide-react";
...
export function PlatformHeader() {
  ...
  const desktopLinks = [
    ...
    { href: "/variants", label: "Варианты" },
    { href: "/workspace", label: "Кабинет" },
    ...(user?.role === "admin" ? [{ href: "/admin/tasks/control", label: "Контент" }] : []),
  ];
  const cabinetLinks = [
  ...
  return <header className="app-header sticky top-0 z-50 border-b border-white/10 bg-[#0b0b0d]/85 backdrop-blur-xl"><div className="container flex h-[74px] items-center justify-between gap-3"><Link href="/" className="group flex min-w-0 items-center gap-3 text-[#f7f2eb]"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#ff5b14] shadow-[0_0_32px_rgba(255,91,20,.33)] transition-transform duration-200 group-hover:rotate-[-6deg] group-hover:scale-105"><BookOpenCheck className="h-5 w-5 text-[#101014]" strokeWidth={2.7} /></span><span className="leading-none"><span className="block font-['Space_Grotesk'] text-[15px] font-bold tracking-[-0.05em]">Школа 911</span><span className="mt-1 block text-[9px] font-extrabold uppercase tracking-[0.22em] text-[#8e8b91]">ОГЭ · МАТЕМАТИКА</span></span></Link><nav className="hidden items-center gap-1 md:flex" aria-label="Основная навигация">{desktopLinks.map(link => <Link key={link.href} href={link.href} className={`rounded-lg px-3 py-2 text-sm font-bold transition-colors ${isActiveRoute(location, link.href) ? "bg-[#ff5b14] text-[#101014]" : "text-[#a7a4aa] hover:bg-white/8 hover:text-white"}`}>{link.label}</Link>)}</nav><div className="flex shrink-0 items-center gap-2"><Button onClick={toggleTheme} variant="ghost" size="icon" className="rounded-lg text-[#f5f0e9] hover:bg-white/10 hover:text-white" aria-label={theme === "dark" ? "Включить светлую тему" : "Включить тёмную тему"} title={theme === "dark" ? "Светлая тема" : "Тёмная тема"}>{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</Button><Link href="/workspace" className="hidden sm:block"><Button variant="secondary" size="sm" disabled={loading} className="gap-2 rounded-lg border border-white/12 bg-white/6 font-bold text-[#f5f0e9] hover:bg-white/10"><UserRound className="h-4 w-4" />{isAuthenticated ? "Кабинет" : "Войти"}</Button></Link>{!isAuthenticated && !loading ? <Button onClick={startLogin} size="sm" className="hidden rounded-lg bg-[#ff5b14] font-extrabold text-[#111113] hover:bg-[#ff7a35] sm:inline-flex">Начать</Button> : null}<Sheet open={menuOpen} onOpenChange={setMenuOpen}><SheetTrigger asChild><Button type="button" variant="outline" className="inline-flex h-10 gap-1.5 rounded-lg border-white/12 bg-white/6 px-2.5 text-xs font-extrabold text-[#f5f0e9] hover:bg-white/10 hover:text-white md:hidden" aria-label="Открыть меню разделов"><Menu className="h-4 w-4" /><span>Меню</span></Button></SheetTrigger><SheetContent side="left" className="w-[min(88vw,340px)] gap-0 border-white/10 bg-[#111113] p-0 text-[#f5f0e9]"><SheetHeader className="border-b border-white/8 px-6 py-5 pr-14 text-left"><SheetTitle className="flex items-center gap-2 text-[#f5f0e9]"><Menu className="h-4 w-4 text-[#ff7a35]" />Разделы школы</SheetTitle><SheetDescription className="text-[#9b989f]">Быстрый переход по доступным разделам.</SheetDescription></SheetHeader><nav className="flex-1 overflow-y-auto px-4 py-4" aria-label="Мобильная навигация">{mobileGroups.map(group => <section key={group.title} className="border-b border-white/8 py-3 first:pt-0 last:border-0"><p className="px-3 pb-2 text-[10px] font-extrabold uppercase tracking-[.15em] text-[#77747b]">{group.title}</p><div className="space-y-1">{group.items.map(item => { const active = isActiveRoute(location, item.href); const Icon = item.icon; return <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold transition ${active ? "bg-[#ff5b14] text-[#101014]" : "text-[#c7c3ca] hover:bg-white/7 hover:text-white"}`}><Icon className="h-4 w-4" />{item.label}</Link>; })}</div></section>)}</nav></SheetContent></Sheet></div></div></header>;
}
