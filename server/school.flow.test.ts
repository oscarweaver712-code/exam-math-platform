...
describe("public bank and tutor homework flow", () => {
  ...
  it("returns only a verified published OGE fixture to a visitor without authentication", async () => {
    ...
    expect(listing.items.every(item => Number.isInteger(item.catalogNumber) && item.catalogNumber > 0)).toBe(true);
    expect(new Set(listing.items.map(item => item.catalogNumber)).size).toBe(listing.items.length);
  ...
  it("keeps source metadata and the archive lifecycle under administrator control", async () => {
    ...
    expect(createdTask.catalogNumber).toBe(createdTask.taskId);
    ...
    await expect(adminCaller.school.admin.getTask({ taskId: createdTask.taskId })).resolves.toEqual(expect.objectContaining({ internalId: createdTask.internalId, catalogNumber: createdTask.taskId, title: "Редакционная задача с источником — обновлена" }));
    const authorTask = await adminCaller.school.admin.createTask({ ...taskInput, title: "Авторская задача для immutable ID", slug: `author-id-${suffix}`, sourceKind: "author", sourceTitle: "Авторская тренировочная задача Школы 911", sourceUrl: undefined, sourceRecordId: undefined, additionalMaterials: [{ title: "Дополнительное условие", bodyMarkdown: "Используйте это уточнение только после нажатия на кнопку раскрытия." }] });
    ...
    await adminCaller.school.admin.addTaskDiagram({ taskId: authorTask.taskId, placement: "supplement", diagramKey: "triangle-angle-48-67", altText: "Авторская схема для дополнительного условия" });
    await expect(publicCaller.publicBank.getTaskMaterials({ taskId: authorTask.taskId })).resolves.toEqual(expect.objectContaining({ materials: expect.arrayContaining([expect.objectContaining({ title: "Дополнительное условие" })]), visuals: expect.arrayContaining([expect.objectContaining({ placement: "supplement", diagramKey: "triangle-angle-48-67" })]) }));
